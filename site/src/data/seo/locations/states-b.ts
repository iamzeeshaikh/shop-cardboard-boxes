...
<p>Practically that means Edge Crush Test rather than Mullen, and it means fit matters more than usual: a product that can move will move for a very long time. A fitted insert is often a better investment than a heavier board.</p>`,
  ...
  faqs: [
    ...
    { q: 'What tear-line placement do grocery buyers usually require?', a: 'A perforation that opens the case cleanly with one pull and leaves a tray whose front panel still holds its shape. The common failure is a tear line set too high, which looks tidy in a drawing and lets product tip forward on a shelf. Send the retailer\'s planogram height and we will set the line from that rather than from the box.' },
    { q: 'Can a single case double as both shipper and shelf tray?', a: 'Yes, and that is what shelf-ready packaging is. The trade-off is that one structure serves two jobs and does neither perfectly — a case strong enough for a full pallet is heavier than a display needs to be. Where the pallet is short and the shelf life is long, splitting them into two structures is sometimes cheaper overall.' },
    { q: 'Do Chicago retailers require a specific case count on the outer?', a: 'Case count, gross weight and a scannable code are close to universal, though the placement rules vary by chain and are the part most often missed. Send the receiving guide with your artwork; the difference between chains is usually where the code sits, not what it contains.' },
    { q: 'How does rail dwell affect a food carton specifically?', a: 'The load sits longer than the transit time suggests, and a carton loses strength while holding weight rather than while moving. For food that means the coating has to survive a temperature cycle as well as the journey, because condensation forms and dries repeatedly in an unheated container.' },
    { q: 'Can you match a board colour across two different product lines?', a: 'Across the same board, yes. Across kraft and a coated white, no — the substrate shows through and the same ink reads differently on each. Where two lines have to look related and one needs kraft, matching a spot colour on both is more reliable than matching the board.' },
  ],
  shop: [
...
<p>For heavier assemblies, a <a href="/product/cardboard-heavy-duty-shipping-crates/">corrugated crate</a> handles several hundred pounds strapped to a pallet, weighs less than timber, and is recyclable at the receiving end rather than a disposal cost.</p>`,
  ...
  faqs: [
    ...
    { q: 'How do you decide between an insert and a heavier board?', a: 'By where the failure happens. If parts arrive marked or scuffed, that is contact between components and an insert fixes it; a heavier board changes nothing. If the carton itself is crushing, that is compression and the board grade is the answer. Sending a photograph of a failed shipment usually settles which one it is.' },
    { q: 'Can one insert hold parts that vary slightly between batches?', a: 'Within a millimetre or two, yes — corrugated has enough give to absorb small variation. Beyond that the cavity either grips the large parts or lets the small ones move. Where variation is genuinely wide, a divider grid sized to the largest part with a foam pad is more tolerant than a die-cut cavity.' },
    { q: 'What does switching from returnable totes to corrugated actually cost?', a: 'You trade a capital item and a return leg for a per-unit cost. Corrugated wins where the return journey is long, where the tote fleet needs washing, or where volumes swing seasonally and idle totes cost storage. It loses on a short closed loop running at steady volume.' },
    { q: 'Do you supply against a released production schedule?', a: 'Yes. Agree the annual quantity, then call it off against your build schedule. The unit price is set by the total rather than by each release, and the schedule is priced at quote stage because it affects freight rather than the box.' },
    { q: 'How should a corrugated crate be closed for a heavy component?', a: 'Strapping through the base rather than tape across the top. Tape holds a lid closed; it does not carry load. On a heavy crate the weight has to reach the pallet through the corner posts, and strapping is what keeps the walls in contact with them.' },
  ],
  shop: [
...
<p>Anything shipped rather than trucked short-haul changes the calculation, because the box holds temperature for longer than the route was planned around. An <a href="/product/cardboard-insulated-cold-boxes/">insulated cold box</a> with route-sized coolant is the specification there.</p>`,
  ...
  faqs: [
    ...
    { q: 'How is a corrugated export crate priced against a wooden one?', a: 'Lower on the unit, and lower again on freight because it weighs a fraction of the timber. The saving that customers usually miss is the process one — no treatment, no marking and no certificate to obtain before the container closes.' },
    { q: 'What board suits a poultry carton that sees repeated condensation?', a: 'A wet-strength board with a dispersion coating on the inside face. The cycle matters more than any single exposure: board that dries and re-wets repeatedly loses more than board that gets wet once, so the grade is chosen for the number of cycles rather than the peak humidity.' },
    { q: 'Can you deliver to a freight forwarder rather than to our site?', a: 'Yes. Give us the forwarder\'s address and reference at quote stage. Forwarders usually have receiving windows and pallet-height limits that differ from a warehouse, so tell us both and we will build the pallet to suit rather than have it turned away.' },
    { q: 'Does a box stored through a Georgia summer need a higher grade?', a: 'If it holds weight while stored, yes. A box sitting empty on a rack recovers most of its strength when the air dries. A box holding a stacked load through that period does not, because it deforms while weakened and keeps the deformation afterwards.' },
    { q: 'Are hand holes a problem on an export crate?', a: 'They cost stacking strength wherever they sit, and on an export crate that strength is already spoken for. Where they are needed, placing them high on the side wall keeps them away from the corner posts that carry the load — and the grade goes up one step to pay for the cut.' },
  ],
  shop: [
...
<p>Where garments ship direct to consumers, a resealable <a href="/product/cardboard-mailing-boxes/">mailer</a> is worth the small unit premium, because apparel return rates make reclosability a real operational saving.</p>`,
  ...
  faqs: [
    ...
    { q: 'What is the largest single-piece carton you can produce?', a: 'The limit is the board width on the corrugator rather than any rule about boxes. Beyond that width a carton is built as two pieces — a base and a telescoping lid, or a tray and sleeve — which is usually stronger than a single piece of that size would have been anyway.' },
    { q: 'How is a furniture carton protected at the corners?', a: 'With corner pads or an integral die-cut post rather than more board on the panels. Furniture damage is almost always a corner impact, and panel strength does nothing for it. The pad transfers the blow into the frame of the carton rather than into the item.' },
    { q: 'What packaging suits a textile mill shipping rolled goods?', a: 'A tube or a long telescoping carton, with the support along the roll rather than at the ends. Rolled fabric supported only at the ends sags in the middle and takes a crease that does not come out, which is the damage claim we see most on this work.' },
    { q: 'Do laboratory shipments need documentation with the packaging?', a: 'Where a protocol governs the shipment, the packaging usually has to be recorded rather than certified — dimensions, board grade and insert design held on file so that a later run matches. We keep the approved dieline and can supply the specification sheet whenever it is asked for.' },
    { q: 'Can folded garments ship in a mailer rather than a carton?', a: 'For one or two items, yes, and it costs less to freight. Beyond about three the mailer loses its shape and the garments crease at the fold. A shallow carton sized to the folded footprint holds the stack flat and photographs better on arrival.' },
    { q: 'Should an oversized carton ship flat or assembled?', a: 'Flat, in nearly every case. An assembled carton of that size is mostly air on the freight invoice, and it arrives creased at the folds from handling. Flat-packed with an assembly sheet costs less to ship and reaches you square.' },
  ],
  shop: [
...
<p>Ordering a season's requirement in one run rather than in weekly lots is where the unit price moves, and the <a href="/product-category/wholesale-cardboard-boxes/">wholesale cardboard boxes page</a> sets out where those breaks fall.</p>`,
  ...
  faqs: [
    ...
    { q: 'Does a box stored in an unheated warehouse need a different specification?', a: 'Cold air alone is not the problem — dry cold barely affects corrugated. What matters is the cycle: a box that warms in daylight and cools overnight collects condensation each time, and repeated wetting is what costs the strength. Insulated storage or a coated board handles it.' },
    { q: 'How quickly can a repeat order be produced from a dieline on file?', a: 'Production is normally 8–10 business days, and holding the dieline removes the approval step rather than the production time. Where a run is genuinely urgent, telling us at quote stage lets us schedule it rather than expedite it, which costs less.' },
    { q: 'What causes an automotive parts carton to fail most often?', a: 'Contact between parts rather than crushing of the box. Metal components that touch mark each other long before the carton is at risk, so the fix is a divider or a die-cut insert. Board grade only becomes the answer when cartons are arriving deformed rather than contents arriving marked.' },
    { q: 'Can an orchard order once and take delivery across the picking season?', a: 'Yes, and it is the usual arrangement here. The full quantity sets the unit price and the deliveries follow the picking schedule, so the packing shed is not full of boxes before the fruit is ready.' },
    { q: 'Does a printed produce box cost much more than a plain one?', a: 'One or two colours on kraft adds little at seasonal quantities, and the plate cost is spread across the run. Full-colour print on a coated board is a different order of cost, and on a box that carries fruit for a week it rarely earns its place.' },
  ],
  shop: [
    { path: '/product/cardboard-industrial-parts-packaging/', label: 'Industrial parts packaging' },
