...
<p>That usually points at a smaller box with an insert rather than a bigger box with filler, which is cheaper on the carrier bill as well.</p>`,
  ...
  faqs: [
    ...
    { q: 'Why does a long roll box need more than a longer carton?', a: 'Because span is the problem, not length. A hoagie box that flexes in the middle tips its contents; the fix is a partition or a scored rib rather than a heavier board. The same box in a shorter format would never need it.' },
    { q: 'Does a window panel weaken a sandwich box?', a: 'It removes load-bearing board from the face it sits on, so a windowed lid takes less top load than a plain one. On a box that is carried rather than stacked this rarely matters. On a box that goes into a display fridge under others, it does.' },
    { q: 'Can the same carton run on two different presses across a year?', a: 'It can, but the colour will not match closely enough for a product range sitting together. Where consistency between batches is the requirement, the run stays on one process, and a spot ink rather than a build gives the best chance of matching.' },
    { q: 'What lead time suits a Philadelphia restaurant group opening a site?', a: 'Production is normally 8–10 business days after artwork approval, plus transit. Opening dates move and printing does not, so ordering the first run against a confirmed date with a week of contingency is the practical approach.' },
    { q: 'Does a compact delivery radius change the barrier requirement?', a: 'It reduces how long grease and steam have to work, so a lighter coating often suffices. It does not change the structural requirement, because the number of handling events on a dense route can be higher than on a long one.' },
  ],
  shop: [
...
<p>A folding carton with a fitted insert weighs less and travels flatter than a rigid box. A <a href="/product/cardboard-rigid-lid-base-boxes/">rigid lid and base box</a> reads as a gift and holds its shape but costs more and cannot be flattened. Which is right depends on whether the box is the gift or merely carries it.</p>`,
  ...
  faqs: [
    ...
    { q: 'What actually makes a catering box quick to assemble?', a: 'A crash-lock base, which arrives glued and opens into shape with one movement. A standard tuck base has to be folded and tucked, and over a few hundred boxes before service that difference is measured in staff hours rather than cents.' },
    { q: 'How does a catering tray hold weight in a transport rack?', a: 'Through the corners, provided the lid does not deflect into the food below. A reinforced rim or a double-thickness edge takes the weight of the tray above without transferring it into the contents.' },
    { q: 'Can institutional contracts be priced annually and drawn down monthly?', a: 'Yes. The annual quantity sets the unit price and the deliveries follow the schedule. This is the arrangement most institutional buyers use, and it is agreed at quote stage because the schedule affects freight rather than the box price.' },
    { q: 'What gift packaging survives being packed in a suitcase?', a: 'A rigid box or a folding carton with a fitted insert — something that holds its shape when compressed between clothing. Sleeves and lightweight cartons arrive creased, and the crease is what the recipient sees before anything else.' },
    { q: 'Can a venue or event name be added to an otherwise standard box?', a: 'Yes. Where the structure stays the same, adding a name is an artwork change rather than a tooling one, so it costs little. Running several names as separate short runs from one dieline is the usual way venues handle this.' },
  ],
  shop: [
...
<p>For temperature-controlled material, an <a href="/product/cardboard-insulated-cold-boxes/">insulated box</a> with coolant sized to the route. For shock-sensitive instruments, an inner box suspended inside an outer — one of the few cases where foam genuinely earns its cost.</p>`,
  ...
  faqs: [
    ...
    { q: 'Does a carrier need a different grid for cans than for bottles?', a: 'Yes. A can is a straight cylinder, so the cell is diameter plus a couple of millimetres. A bottle tapers, so the cell must hold base and neck separately or the bottle rocks. A grid designed for one holds the other badly.' },
    { q: 'What happens to a beverage case in a cold store?', a: 'It condenses, because anything brought in warm collects moisture. The grade has to be chosen for the damp state the box will actually carry the load in rather than the dry rating it arrives with.' },
    { q: 'Can a laboratory specification be reproduced years later?', a: 'Yes, provided the dieline and specification are held on file — which is what we do as standard. The later run is produced against the stored version rather than re-specified from a description, which is where drift creeps in.' },
    { q: 'What is the specific failure risk in coastal seafood packaging?', a: 'Repeated wetting rather than a single soaking. Salt air, melting ice and washdown all cycle the board through wet and dry, and each cycle costs strength that does not come back. Wet-strength board and a dispersion coating handle it.' },
    { q: 'Can a beverage carrier be printed on the handle panel?', a: 'Yes, and it is the panel most often left blank. It is the face a customer looks at while carrying the pack from a taproom, and it prints in the same pass as everything else.' },
  ],
  shop: [
...
<p>Client gifting asks the same structure to look more expensive than it costs, which is what <a href="/product/cardboard-corporate-gift-packaging/">corporate gift packaging</a> is finished for.</p>`,
  ...
  faqs: [
    ...
    { q: 'Why do display units collapse in store rather than in transit?', a: 'Because they hold weight for weeks under warm lights rather than for days in a truck. Corrugated creeps under sustained load, and a unit strong enough to arrive intact can still sag by week three. The grade is specified for the dwell, not the journey.' },
    { q: 'Should a display ship flat or assembled to a store?', a: 'Flat, unless store staff cannot assemble it. Flat costs a fraction to freight and arrives square. Assembled units arrive creased at the folds from handling, and the freight volume on a full rollout is substantial.' },
    { q: 'How is colour kept consistent across a rollout of several hundred stores?', a: 'By running the whole rollout at once on one process, with a spot ink rather than a four-colour build. Splitting a rollout across production dates is the most common cause of two stores looking different.' },
    { q: 'Can deliveries go to individual store receiving bays?', a: 'Yes, though most rollouts go to a distribution centre and are broken down there, which is cheaper. Where direct store delivery is needed, give us the addresses and any receiving windows at quote stage.' },
    { q: 'What lead time does a multi-store rollout need?', a: 'Production of 8–10 business days, plus transit, plus time for a physical sample to be approved before the full run. It is the sample step that decides whether a rollout is comfortable or rushed.' },
  ],
  shop: [
...
<p>Where the outer can stay plain, <a href="/product/cardboard-box-inserts/">fitted inserts cut to the device</a> carry the protection on their own.</p>`,
  ...
  faqs: [
    ...
    { q: 'What should a first packaging run actually include?', a: 'One structure, in the smallest quantity that lets you see it in the hand and photograph it. At 100 units the unit price is high and the total is small, which is the right trade for a first run. Commit to volume after the design has survived a real shipment.' },
    { q: 'How does a subscription box stay cheap across monthly cycles?', a: 'By keeping one dieline and changing only the print, or by using a plain box with printed sleeves. The tooling is the fixed cost; artwork is the variable one. Redesigning the structure each cycle is what makes subscription packaging expensive.' },
    { q: 'What does a hardware device need that a general box does not?', a: 'An insert built to the device outline including any protrusions, and a printed interior if the unboxing is part of the product. The corrugated carries the shock; the insert decides whether the device ever touches a wall.' },
    { q: 'Can packaging be produced to a specific launch or event date?', a: 'Yes, working back from the date through transit and 8–10 business days of production. Send dimensions before the artwork is settled so the dieline can be cut in parallel — that is usually where the time is found.' },
    { q: 'Is it worth committing to a year of boxes for the price?', a: 'If the design is settled, yes, and call-off delivery means you do not store them. If the design is still moving, no — the saving rarely covers the cost of obsolete stock after a rebrand.' },
  ],
  shop: [
