...
<p><a href="/resources/corrugated-box-strength-guide/">The strength guide</a> covers how ECT figures translate into real stacking limits.</p>`,
  ...
  faqs: [
    ...
    { q: 'What does removing the timber paperwork actually save on a shipment?', a: 'A treatment step, a stamp and a certificate, plus the delay of arranging them. On a schedule where a container has a booked sailing, that process time is often worth more than the difference in material cost.' },
    { q: 'How is a corrugated crate closed for ocean freight?', a: 'Strapped through the base rather than taped across the top, with the strapping bearing on the corner posts. Tape holds a lid shut; it does not carry load, and an ocean journey applies load from directions a road journey does not.' },
    { q: 'Does a container\'s humidity affect boxes even in a dry destination?', a: 'Yes, because the humidity travels with the load. The air sealed in at departure condenses when the container cools at sea, regardless of the climate at either end. Desiccant and a coated board handle it together.' },
    { q: 'What is a realistic weight ceiling for a corrugated export crate?', a: 'It depends on the load path rather than a single figure. A load spread across the base goes far higher than a concentrated one. Send the gross weight together with the contact area and we will specify against both rather than guess from the weight alone.' },
    { q: 'Can you deliver to a port terminal rather than to a warehouse?', a: 'Yes, with the terminal reference and any booking window given at quote stage. Terminals receive differently from warehouses and a delivery arriving outside a window is turned away rather than held.' },
  ],
  shop: [
...
<p>The commonest error is specifying to the total weight when the load is concentrated. A dense part resting on one corner fails well below the box rating — an insert that spreads the load is the fix.</p>`,
  ...
  faqs: [
    ...
    { q: 'Where should hand holes go on a heavy agricultural sack box?', a: 'High on the side walls, away from the corners. The corner posts carry the stack, and a hole cut near them costs far more strength than the same hole placed centrally. The grade normally goes up one step to pay for whichever holes are needed.' },
    { q: 'How is a part with protruding fittings packaged?', a: 'By building the insert around the fittings rather than the body. Protrusions concentrate the entire weight onto a small area of wall, which is why they puncture boxes that the flat faces would never trouble.' },
    { q: 'Can a feed or seed supplier order one season at a time?', a: 'Yes, and it is the sensible way to buy here. The season\'s quantity sets the price and deliveries follow the schedule, so the shed is not full before the season starts.' },
    { q: 'Why would a box fail below its stated weight rating?', a: 'Almost always because the load is concentrated rather than spread, or because the box was damp when it took the weight. The rating assumes an even load on dry board, and real shipments frequently satisfy neither condition.' },
    { q: 'Are dielines kept on file for traceable reorders?', a: 'Yes. Every approved dieline is stored, and repeat runs are produced against the stored version. For precision components, that is what makes a second batch match the one that was qualified.' },
  ],
  shop: [
...
<p>Three or four sizes covering roughly a quarter of orders each is where most apparel operations land. Because carriers bill on dimensional weight, a shallow rectangle beats a cube for folded garments on both fit and freight. <a href="/resources/cardboard-box-sizes-guide/">The sizes guide</a> covers drawing the cut lines from real order data.</p>`,
  ...
  faqs: [
    ...
    { q: 'What makes a mailer genuinely resealable rather than merely reusable?', a: 'A second adhesive strip and a tear line placed so the box still closes after the first opening. Without the second strip a customer is taping it shut, which is the point at which most returns get packed badly.' },
    { q: 'Does a returns-friendly box cost more overall?', a: 'Usually less. The strip and tear line add a few cents; a returns process that needs a second box adds a whole box plus the labour to pack it. The maths favours the strip in almost any operation with a real return rate.' },
    { q: 'How many sizes should a fulfilment box ladder hold?', a: 'Enough that most shipments fill their box and few enough that no size runs in small quantities. A week of shipment dimensions shows both ends of that — the sizes shipping half empty, and the sizes barely used.' },
    { q: 'Why does apparel end up in shallow boxes rather than cubes?', a: 'Because folded clothing is wide and flat, and a cube lets the stack shift and crease. A shallow box sized to the folded footprint also bands lower on dimensional weight, so it costs less to ship as well as arriving better.' },
    { q: 'Can deliveries follow a monthly schedule rather than arriving at once?', a: 'Yes. Agree the annual quantity for the price and take delivery monthly. Tell us the pattern at quote stage, because the schedule is priced into the freight rather than the unit cost.' },
  ],
  shop: [
...
<p>Where garments are shipped in bulk to retailers rather than to consumers, the specification shifts to stacking — Edge Crush Test rather than puncture resistance — and <a href="/product/cardboard-shelf-ready-packaging/">shelf-ready cases</a> if the retailer requires them.</p>`,
  ...
  faqs: [
    ...
    { q: 'What holds a multi-item kit in place without foam?', a: 'A die-cut corrugated insert with a cavity per item. It costs less than foam, recycles with the box, and holds a fixed arrangement so every kit opens the same way — which matters more for a gift than the cushioning does.' },
    { q: 'Can individual kits be personalised for each recipient?', a: 'The printed box cannot economically change per recipient, but the contents and an inserted card can. Where names must appear on the box, digital printing makes short variable runs possible at a cost that rises quickly with the number of variants.' },
    { q: 'What quantity makes a corporate gifting run worthwhile?', a: 'From 100 upward. Below a few hundred the unit price is dominated by setup, so the practical question is usually whether one design across all recipients beats several smaller runs — and it usually does.' },
    { q: 'What case information does a retail buyer expect on the outer?', a: 'A case count, a gross weight and a scannable code, positioned where that chain expects to find them. The content is near-universal and the placement is not, so send the receiving guide with your artwork rather than after the first delivery.' },
    { q: 'Can a kit insert be changed without re-tooling the outer box?', a: 'Yes, and it is the cheaper way to vary a gifting programme. The outer stays on one dieline while the insert is re-cut for each contents list, so a seasonal change costs an insert rather than a box.' },
  ],
  shop: [
...
<p>Where several designs are needed, keeping one box structure and varying a printed <a href="/product/cardboard-sleeve-packaging/">sleeve</a> avoids retooling for every variant.</p>`,
  ...
  faqs: [
    ...
    { q: 'Is redesigning a box worth it just to change dimensions?', a: 'If the change moves the shipment into a lower carrier band, almost always. The tooling is a one-off and the saving repeats on every unit shipped. If it does not cross a band, the redesign saves board area and little else.' },
    { q: 'How is a box ladder tested before committing to it?', a: 'Against real shipment data rather than a catalogue. A week of actual dimensions run against the carrier\'s band table shows which sizes pay for themselves. Guessing at the ladder is how operations end up with sizes nobody uses.' },
    { q: 'Why does regulated print stay on offset rather than digital?', a: 'Because a batch code and a regulated symbol must read identically on every carton, and offset with a spot ink reproduces exactly where a digital simulation drifts slightly between runs.' },
    { q: 'How late can artwork arrive for an event merchandise run?', a: 'Later than most people assume, provided the dieline is already cut. Structure is what takes the time; artwork can arrive close to the production slot. Send dimensions early even if the design is unresolved.' },
    { q: 'What is the cheapest way to run several merchandise designs?', a: 'One dieline, several print runs — or a plain box with printed sleeves where the designs ship together. Both avoid paying tooling more than once, which is where multi-design runs usually overspend.' },
  ],
  shop: [
