...
<p>The second is that a large share of California freight moves through the ports at Los Angeles, Long Beach and Oakland. Export shipments sit in humid conditions for weeks, and a corrugated crate avoids the heat-treatment requirements that apply to solid wood packaging — worth knowing before specifying timber. See <a href="/product/cardboard-heavy-duty-shipping-crates/">heavy-duty shipping crates</a>.</p>`,
  ...
  faqs: [
    ...
    { q: 'Are your boxes recyclable in California kerbside collection?', a: 'Plain corrugated and boxes finished with a water-based dispersion coating stay in the paper stream and are accepted as cardboard. Wax and polyethylene laminates are not — they have to be separated first, and most households will not do it. If the recycling claim matters to your brand, say so at quote stage and we will keep the specification to a single material.' },
    { q: 'Can a Central Valley grower order for one harvest window rather than a full year?', a: 'Yes. Produce buying is seasonal and there is little advantage in holding boxes through the off-season, where humidity cycles cost strength anyway. Order the season\'s requirement in one production run for the unit price, and we can schedule delivery across two or three drops so the shed is not full in week one.' },
    { q: 'What changes when a beauty box ships monthly instead of once?', a: 'The failure mode moves from transit damage to inconsistency. A subscription customer sees twelve boxes a year and notices when the twelfth is a slightly different white from the first. That points at a coated board and a spot ink rather than a digital simulation, and at keeping the same approved dieline on file between runs.' },
    { q: 'Can you print barcodes or carton labels directly onto the box?', a: 'Yes, as part of the print rather than as an applied label. Send the symbology and the quiet-zone requirement with your artwork. Scanning reliability depends more on contrast and on flute direction beneath the code than on the printing method, so tell us which face gets scanned and we will orient the board around it.' },
    { q: 'Does the unit price change much between 100 boxes and 1,000?', a: 'It falls in steps rather than smoothly, because setup cost spreads across the run while board cost does not. The steepest part of that curve is in the first few hundred units. Beyond a few thousand the price flattens, and further saving comes from board choice or from cutting the box down to size rather than from quantity.' },
  ],
  shop: [
...
<p>Summer heat compounds it. Sustained warmth in a trailer softens adhesives and, combined with humidity on the Gulf side, reduces stacking strength over a multi-day journey. For loads that sit before they are unpacked, specifying a grade up is cheap insurance.</p>`,
  ...
  faqs: [
    ...
    { q: 'Which board grade suits a load crossing Texas in July?', a: 'One grade above what the same load would need in November, if it sits on a pallet before it is unpacked. Sustained cab-level heat softens adhesive and Gulf humidity does the rest, and stacking strength falls with both. The extra grade costs a few cents a box against a load that arrives leaning.' },
    { q: 'Do corrugated crates need ISPM 15 marking for export from Texas ports?', a: 'No marking is needed. ISPM 15 applies to solid wood packaging, and corrugated crates fall outside it entirely — that exemption is one of the main reasons exporters move to them. There is no heat treatment, no stamp and no certificate to chase before a container closes.' },
    { q: 'How do you stop a barbecue box going soft on a long drive?', a: 'Vent it high and coat it for fat rather than for water. A dispersion coating handles the grease, and vents placed high on the side walls let steam leave before it condenses on the lid and runs back into the board. Where the holes sit matters more here than the board grade does.' },
    { q: 'Can you supply the same box to several Texas locations?', a: 'Yes, and it is worth doing deliberately. One approved dieline produced in a single run gives every site an identical box, which matters when staff move between sites and when a display is set the same way in each. Split delivery is priced at quote stage rather than added afterwards.' },
    { q: 'Is a corrugated crate strong enough for oilfield components?', a: 'That depends on the load path rather than the weight alone. Corrugated carries compression through the corners well and point loads badly, so a heavy component resting on a flat panel needs an insert that transfers the weight into the corner posts. Send the component dimensions and gross weight and we will specify against them.' },
  ],
  shop: [
...
<p>That rewards packaging that is light, that protects without bulk, and that survives being sat on. A <a href="/product/cardboard-rigid-lid-base-boxes/">rigid lid and base box</a> reads as a gift and holds its shape; a folding carton with a fitted insert weighs less and travels flatter. Which one is right depends on whether the box is the gift or just carries it.</p>`,
  ...
  faqs: [
    ...
    { q: 'How long can a box sit in Florida humidity before strength becomes a problem?', a: 'Corrugated reaches moisture equilibrium with the air around it within a day or two, and at that point holds roughly two-thirds of its dry stacking strength. The loss is not progressive after that, so a box stored six weeks is not weaker than one stored six days. It is the first exposure that costs the strength, not the duration.' },
    { q: 'Does a moisture-resistant coating stop the box being recycled?', a: 'A water-based dispersion coating does not — it repulps with the fibre and the box goes in as cardboard. A wax or polyethylene laminate does, because it has to be separated first. For most Florida applications the dispersion coating is enough, and it is what we specify unless the load is genuinely saturated.' },
    { q: 'What ventilation pattern suits citrus shipped in warm weather?', a: 'Vents on the side walls rather than the ends, placed high, and aligned between layers so airflow continues up the stack rather than stopping at the first box. Field heat has to leave through the stack, and a vent pattern that does not line up between layers moves almost no air at all.' },
    { q: 'Can you deliver to the Keys or a barrier-island address?', a: 'Yes, though those routes add transit days and are the first to pause under a storm advisory. Give us the delivery address at quote stage rather than at order stage — freight to these addresses is priced differently, and we would rather show it upfront than adjust an invoice later.' },
    { q: 'Do you make packaging for a Florida resort or beach retail range?', a: 'Yes. The constraint here is usually humidity in an open-fronted store rather than transit: a printed carton left in that air softens and curls at the edges within a season. A heavier board with a coating on the printed face holds its shape long enough to sell through.' },
  ],
  shop: [
...
<p>Which barrier to specify depends on butter content more than on the bake, and <a href="/resources/food-packaging-box-guide/">the food packaging guide</a> compares the coatings that handle it.</p>`,
  ...
  faqs: [
    ...
    { q: 'How small a box can you produce for a jewellery or accessory line?', a: 'Down to a few centimetres on each side. Below roughly 40 mm the constraint stops being the box and becomes the insert — a pad or platform has to hold the piece without pressing on it, and at that scale the tolerance on the cavity is tighter than the tolerance on the box.' },
    { q: 'Does a resealable mailer cost much more than a plain one?', a: 'The second adhesive strip and the tear line add a few cents per unit at typical quantities. Against that, a returns process that does not need a second box is usually cheaper overall — you pay once for packaging that makes two journeys rather than twice for packaging that makes one each.' },
    { q: 'Can you produce for a New York brand shipping from a New Jersey warehouse?', a: 'Yes, and it is the common arrangement. Boxes go to whichever address receives them, which is often not the address on the invoice. Tell us the delivery point at quote stage so the freight is priced to where the pallet actually lands.' },
    { q: 'What lead time should a New York retail launch allow?', a: 'Work back from the on-shelf date through production, transit and a week of contingency. Production is normally 8–10 business days from artwork approval, and it is artwork approval rather than production where most launches lose time. Send dimensions before the artwork is finished and we can start the dieline in parallel.' },
    { q: 'Do you print inside the box as well as outside?', a: 'Yes, and for direct-to-consumer brands it is usually the better place to spend. The outer surface of a shipper is scuffed by the time it arrives and recycled shortly after. The inner face is what the customer sees at the moment they open it, and it is the face that gets photographed.' },
  ],
  shop: [
...
<p>Where the quantity breaks actually fall is a separate question, set out on the <a href="/product-category/wholesale-cardboard-boxes/">wholesale cardboard boxes page</a>.</p>`,
  ...
  faqs: [
    ...
    { q: 'What does a distribution centre usually specify on a carton?', a: 'Edge Crush Test rather than Mullen burst, a stated gross weight, and a case count on the outer. ECT is the relevant number because the failure mode in a warehouse is compression from the stack above rather than puncture. Send the receiving requirements and we will build to them rather than approximate them.' },
    { q: 'Does ordering several box sizes together cost less than separate runs?', a: 'In almost every case, yes. Setup is charged per dieline rather than per order, so several sizes produced together share the scheduling and the freight rather than each carrying its own. The saving is largest where the sizes use the same board grade.' },
    { q: 'Can you produce cartons to a pharmaceutical customer\'s approved artwork?', a: 'Yes. We work to a supplied dieline and colour standard, hold the approved version on file, and produce every later run against it. Where a batch code or regulated symbol appears on the carton, tell us which face it sits on so the board is oriented for legibility rather than for cutting efficiency.' },
    { q: 'How high can cartons stack before the bottom layer is at risk?', a: 'It follows from the ECT rating, the pallet pattern and how long the load sits. A carton loses strength while it holds weight, so a stack that is safe for a week is not necessarily safe for a quarter. Tell us the storage duration alongside the pallet height and we will specify the grade around both.' },
    { q: 'Do you deliver on a schedule rather than all at once?', a: 'Yes. Call-off delivery gives you the price of the full quantity with deliveries spread across an agreed period, which is how most distribution customers buy. It is arranged at quote stage because the schedule affects the freight rather than the unit price.' },
  ],
  shop: [
