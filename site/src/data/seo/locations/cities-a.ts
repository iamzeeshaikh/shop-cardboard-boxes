...
<p>Exactly where the quantity breaks fall, and what changes above them, is covered on the <a href="/product-category/wholesale-cardboard-boxes/">wholesale cardboard boxes page</a>.</p>`,
  ...
  faqs: [
    ...
    { q: 'Why do delivery boxes fail more in Manhattan than elsewhere?', a: 'Because of handling count rather than distance. A box on a nine-stop bike route is lifted, stacked, tilted and re-stacked far more times than one that travels three hundred miles on a pallet. The specification follows the number of handling events, not the mileage.' },
    { q: 'How should a box be specified for an insulated delivery bag?', a: 'For side compression rather than top load. Boxes in a bag are carried at an angle and press against each other sideways, which is not the direction a carton is strongest in. A slightly heavier board and a lid that locks rather than folds handles it.' },
    { q: 'Can a small studio order without a loading dock?', a: 'Yes. Tell us at quote stage that the delivery is kerbside and to a walk-up, because the freight is booked differently and a pallet that cannot be received costs a redelivery. Splitting the run into smaller cartons is usually cheaper than a failed delivery.' },
    { q: 'What is the practical limit on box size for a walk-up delivery?', a: 'Whatever two people can carry up a stairwell, which in practice means keeping cartons under about 15 kg and narrow enough to turn a landing. That constraint often decides the case count before anything about the product does.' },
    { q: 'Does a smaller box always cost less to ship in the city?', a: 'Not for a bike or van route, where the charge is per stop rather than per volume. Smaller boxes pay off in a parcel network. Within the city, packing density inside the bag matters more than the box size on any rate card.' },
  ],
  shop: [
...
<p>Meal prep is the same food on a different schedule, sitting refrigerated for days rather than travelling hot for minutes. <a href="/product/cardboard-meal-prep-boxes/">Meal prep boxes</a> are specified for that dwell time instead.</p>`,
  ...
  faqs: [
    ...
    { q: 'How does a garment box differ from a general mailer?', a: 'It is shallower and wider, sized to the folded footprint rather than to a cube. Folded clothing needs to lie flat; a deep box lets the stack shift and arrive creased. The board can be lighter than a general mailer because the contents weigh little and cushion themselves.' },
    { q: 'What lead time does a promotional or influencer run need?', a: 'Production is normally 8–10 business days, and rigid magnetic boxes take longer because they ship assembled. For a campaign date that cannot move, send dimensions early and let the dieline be cut while the artwork is still being approved.' },
    { q: 'Why do magnetic closure boxes cost more to freight?', a: 'Because they cannot be shipped flat. A folding carton travels as sheets and is assembled where it is used; a rigid magnetic box travels as a box, which is mostly air. On a large run the freight difference can exceed the difference in unit price.' },
    { q: 'What suits a food route that crosses the whole city?', a: 'The same vent-and-coat specification as a short route, with more attention to the lid. Two hours in a warm vehicle condenses far more moisture than twenty minutes does, and it is the lid that collects it and drips it back onto the food.' },
    { q: 'Can a studio order a hundred boxes for a single shoot?', a: 'Yes — 100 is the minimum and it exists for exactly this. The unit price is higher than at volume, but for a one-off shoot or a press run the total is small and the alternative is buying a thousand boxes to use fifty.' },
  ],
  shop: [
...
<p>That points at a separate lift-off lid rather than a taped carton — it reopens indefinitely, and it can be reclosed without tape at the end of a show. For display material, <a href="/product/cardboard-pop-display-boxes/">POP display boxes</a> that ship flat and assemble on site avoid the freight cost of shipping air.</p>`,
  ...
  faqs: [
    ...
    { q: 'What separates a box that ships food from one that delivers it?', a: 'Time and temperature swing. A delivery box lives for thirty minutes in a warm bag; a shipped food box spends two days in a parcel network that may freeze one night and warm the next. The barrier, the coolant and the board grade all change with that.' },
    { q: 'Do trade show cases need to be reusable?', a: 'It depends how many shows the stand does. For a single event, a one-way corrugated case is cheaper and lighter. For a circuit, a heavier case with reinforced corners survives repeated packing, and the cost per show falls quickly after the third one.' },
    { q: 'How is a display case protected in transit to a convention centre?', a: 'By assuming it will be handled by a freight carrier rather than by your team. That means corner protection, a closure that survives strapping, and marking that identifies which way is up without anyone reading the label. Convention freight is stacked by people who do not know what is inside.' },
    { q: 'When is litho lamination worth the extra cost on a retail carton?', a: 'When the print quality is doing sales work — photographic imagery, a fine gradient, or a brand colour that must match across a range. For a case that carries a product to a shelf and is then discarded, flexo does the job for less.' },
    { q: 'Does an unheated Chicago warehouse change the box specification?', a: 'Only where the box cycles between warm and cold. Steady cold is harmless. A dock door opening onto winter air beside a heated space creates exactly the cycle that costs strength, and that is where a coated board earns its cost.' },
  ],
  shop: [
...
<p>Consolidating to four sizes that each cover about a quarter of shipments normally saves more than any negotiation on unit price, and it speeds up the packing bench. <a href="/resources/cardboard-box-sizes-guide/">The sizes guide</a> covers how to draw the cut lines.</p>`,
  ...
  faqs: [
    ...
    { q: 'How is a validated medical pack kept identical between orders?', a: 'By producing every run against a dieline and specification held on file rather than re-specifying each time. Where a protocol names the board grade or insert design, we build to that exactly, and any change is re-approved before production rather than substituted.' },
    { q: 'What information do you need to specify a crate for a heavy component?', a: 'The component outline including any protrusions, the gross weight, and which face it will rest on. The last of those is the one most often left out, and it decides whether the load reaches the corner posts or presses into the middle of a panel.' },
    { q: 'What board suits imported food repacked in Houston humidity?', a: 'One graded for its damp strength, with a dispersion coating where the product releases moisture. The repack environment is often more humid than the storage that follows it, and board that softens during repacking does not recover its rating afterwards.' },
    { q: 'Do you deliver into a port or free-trade zone address?', a: 'Yes. Give us the address and any reference at quote stage, because these sites usually have booking windows and paperwork that a warehouse does not. Pricing it in beforehand is cheaper than a rejected delivery.' },
    { q: 'Can one crate design cover components of different lengths?', a: 'Up to a point. A crate sized for the longest component wastes freight volume on the shorter ones, and an insert sized for the shortest will not hold the longest at all. Where the range is wide, two crate lengths sharing one board grade usually costs less overall than one compromise size.' },
  ],
  shop: [
...
<p>Committing to an annual quantity and taking it on a call-off schedule fixes that: the volume price applies to the whole year, and delivery is heavy before the season and light after it. <a href="/product-category/wholesale-cardboard-boxes/">Wholesale ordering</a> covers how it works.</p>`,
  ...
  faqs: [
    ...
    { q: 'What fails first when a box sits in extreme heat?', a: 'The adhesive, not the board. Tape creeps and flaps lift while the corrugated itself is still within its rating. A hot-melt rated for the temperature, or strapping instead of tape, addresses the actual failure rather than the assumed one.' },
    { q: 'How much coolant does a summer delivery route need here?', a: 'More than the same route elsewhere, sized to the vehicle interior rather than the outside temperature. A closed van in Phoenix runs far above ambient, and coolant calculated against the weather forecast rather than the cab reality runs out early.' },
    { q: 'Does a reflective or light-coloured box help in this climate?', a: 'Marginally, and only in direct sun. Inside a vehicle or a container the air temperature does the work and the box colour makes little difference. Keeping stock out of direct light through a roller door matters more than what is printed on it.' },
    { q: 'How do you avoid small-order pricing during a slow season?', a: 'Order the year at once and take delivery across it. The unit price follows the total quantity rather than each delivery, so a call-off schedule gives you volume pricing without a warehouse full of boxes through the quiet months.' },
    { q: 'Can stock be delivered straight into a distribution centre?', a: 'Yes. Tell us the receiving window, the pallet height limit and any booking reference at quote stage. Freight to an appointment is priced differently from freight to a dock that receives all day.' },
  ],
  shop: [
